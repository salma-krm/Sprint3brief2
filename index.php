...
$sql ="SELECT
    ...
    player.nom, 
    player.rating,
    nationnalite.id AS nationnalite_id,
    statistic_player.id AS statistic_player_id,
    ...
    statistic_gk.id AS statistic_gk_id,
    statistic_gk.diving, 
...
left JOIN statistic_player ON statistic_player.id = player.id_statistic_player
left JOIN statistic_gk ON statistic_gk.id = player.id_statistic_gk";

$result = mysqli_query($coon, $sql);

if($result){
    while($row=mysqli_fetch_assoc($result)){
        $nationnalite_id= $row['nationnalite_id'];
        $equipe_id= $row['equipe_id'];
        $statistic_gk_id= $row['statistic_gk_id'];
        $statistic_player_id= $row['statistic_player_id'];
        $id= $row['id'];
        $name = $row['nom'];
        $logo =$row['logo'];
...
<button  type="button"  class="btn btn-success"><a href="update.php?update='.$id.'update1='.$equipe_id.' update2='.$nationnalite_id.' update3='.$statistic_player_id.'update4='.$statistic_gk_id.' " >update</a></button>
<button  type="button"   class="btn btn-danger"><a href="update.php?delete='.$id.'" >deleted</a></button>
    </td>
    </tr>
   ';
